    ...
    return (
      <div className="p-8 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[1,2,3].map(i => <div key={i} className="h-32 bg-white border border-gray-100 rounded-2xl shadow-sm"></div>)}
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="h-80 bg-white border border-gray-100 rounded-2xl shadow-sm"></div>
          <div className="h-80 bg-white border border-gray-100 rounded-2xl shadow-sm"></div>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Market Insights</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-md transition-shadow">
          ...
          <p className="text-2xl font-extrabold text-gray-900 mt-2">₹{(stats.minPrice / 100000).toFixed(1)} L - ₹{(stats.maxPrice / 10000000).toFixed(1)} Cr</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-md transition-shadow">
          ...
          <p className="text-4xl font-extrabold text-gray-900">{stats.sampleSize}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-gray-800">
            <PieChartIcon size={20} className="text-gray-400" /> Inventory by Locality
          </h3>
          <div className="h-64">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-gray-800">
            <Home size={20} className="text-gray-400" /> Configuration Split
          </h3>
          ...
          <div className="flex justify-center gap-4 flex-wrap mt-2">
            {stats.bhkData.map((entry, i) => (
              <div key={entry.name} className="flex items-center gap-2 text-sm font-medium text-gray-600">
